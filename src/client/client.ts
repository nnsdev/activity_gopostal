...
import { letterboxes } from '../common/letterboxes'
...
const handleZone = (): void => {
  ...
  if (zoneName == null || !letterboxes[zoneName]) {
    return
  }

  const currentZone = jobStatus.zones[jobStatus.currentZone]
  if (currentZone.zone === letterboxes[zoneName].name) {
...
const enteredZone = (currentZone: ZoneData): void => {
  if (!currentZone.isInZone) {
    ...
    global.exports["np-activities"].notifyPlayer(playerServerId, "Deliver the package!")
...
const handleDelivery = (currentZone: ZoneData): void => {
  ...
  global.exports["np-activities"].notifyPlayer(playerServerId, `Box delivered! ${BOXES_PER_ZONE-currentZone.boxesDelivered.length} packages left`)
}
...
global.exports('setLocationStatus', (locationId: number, enabledDisabled: boolean) => {
  const index = Object.keys(letterboxes)[locationId]
  if (index != null) {
    letterboxes[index].enabled = enabledDisabled
  }
})
